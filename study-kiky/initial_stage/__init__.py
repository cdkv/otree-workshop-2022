...
class Subsession(BaseSubsession):
    ...
    assigned_participants = models.IntegerField(initial=0)
...
def group_by_arrival_time_method(subsession: Subsession, waiting_players):
    session = subsession.session
    import random
    if (len(waiting_players) + subsession.assigned_participants) >= C.PLAYERS_IN_SESSION:
        ppg = C.PLAYERS_PER_GROUP
        firms = [p for p in waiting_players if p.role == C.FIRM_ROLE]
        officials = [p for p in waiting_players if p.role == C.OFFICIAL_ROLE]
        minimum_per_role = (C.PLAYERS_IN_SESSION - subsession.assigned_participants) // ppg
        if subsession.round_number == 1:
            new_players = random.sample(waiting_players, ppg)
            update_assigned(subsession)
            return new_players
        elif min(len(firms), len(officials)) < minimum_per_role:
            return
        else:
            if subsession.assigned_participants == 0:
                calculating_averages(subsession)
            new_firm = random.choice(firms)
            new_official = random.choice(officials)
            update_assigned(subsession)
            return [new_firm, new_official]
def update_assigned(subsession: Subsession):
    session = subsession.session
    ppg = C.PLAYERS_PER_GROUP
    ap = subsession.assigned_participants
    if ap + ppg >= C.PLAYERS_IN_SESSION:
        subsession.assigned_participants = 0
    else:
        subsession.assigned_participants += ppg
